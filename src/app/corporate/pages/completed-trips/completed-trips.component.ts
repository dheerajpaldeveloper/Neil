...
export class CompletedTripsComponent implements OnInit {
  ...
  public initTable() {
    ...
    this.httpService
      .postFormData(environment.urlC + "ride_history", payload)
      .subscribe((data) => {
        console.log("colaspss data", data)
        if (typeof (data) == 'string')
          data = JSON.parse(data);
        if (data.flag == 101) {
          this.router.navigate(["/", "corporate_login"]);
        ...
        if (data.flag == 502) {
          ...
          Bookings.forEach((bookingData) => {
            ...
            if (d.ride_status == 'Completed') {
              ...
              this.getRouteEnc(d.pickup_location_address, d.manual_destination_address, d);
            }
  ...
  public getRouteEnc(o, d, bookingData) {
  ...
  public rescheduleNow() {
    ...
    if (!this.rTrip.pickup_id) {
      this.utilityService.alert('error', 'Pickup ID is not found');
      return false;
    }
